...
'216120
'Public Sub LeggiParametriBruciatorePID()
'
'    'Continua a leggere e scrivere su file .ini e non su XML (versione Caronte)
'
...
'    'CYBERTRONIC_PLUS
'    '20160615
'    'FattoreDiCorrezioneKp = CInt(FileGetValue(nomeFile, SEZIONE, "FattoreDiCorrezioneKp", "30"))
'    'TInterventoCampionamento = CInt(FileGetValue(nomeFile, SEZIONE, "TInterventoCampionamento", "15000"))
'    FattoreDiCorrezioneKp = CLng(FileGetValue(nomeFile, SEZIONE, "FattoreDiCorrezioneKp", "30"))
'    TInterventoCampionamento = CLng(FileGetValue(nomeFile, SEZIONE, "TInterventoCampionamento", "15000"))
'    '
'    FattoreDiCorrezioneKd = CDbl(FileGetValue(nomeFile, SEZIONE, "FattoreDiCorrezioneKd", "0"))
'    FattoreDiCorrezioneKi = CDbl(FileGetValue(nomeFile, SEZIONE, "FattoreDiCorrezioneKi", "250000"))
'
''20151209
'    If ListaTamburi(0).AbilitazioneConsumoCombustibile Then
'
'        Dim rs As New adodb.Recordset
'        Dim IdAppoggio As String
'
'            With rs
'                Set .ActiveConnection = DBcon
'                .Source = "SELECT [IdMaterialeLOG],[Nome] FROM [CYB500].[dbo].[MaterialiLOG] WHERE [Categoria]=4;"
...
'                    If IdAppoggio <> "" Then
'                        Do Until .EOF
'                            If CInt(IdAppoggio) = .Fields("IdMaterialeLOG").Value Then Exit Do
'                            .MoveNext
'                            '.Seek
'
'                        Loop
...
'                            .MoveFirst
'                            ListaTamburi(0).IdCombustibileLOG = .Fields("IdMaterialeLOG").Value
'                        Else
'                            'materiale trovato
'                            ListaTamburi(0).IdCombustibileLOG = IdAppoggio
'                        End If
'                    Else
'                        'se il parametro e' nullo assegno il primo combustibile della lista
'                        .MoveFirst
'                        ListaTamburi(0).IdCombustibileLOG = .Fields("IdMaterialeLOG").Value
'                    End If
'
...
'    nomeFile = UserDataPath + FileBruciatore
'
'    Call FileSetValue(nomeFile, SEZIONE, "FattoreDiCorrezioneKp", CStr(FattoreDiCorrezioneKp))
'    Call FileSetValue(nomeFile, SEZIONE, "TInterventoCampionamento", CStr(TInterventoCampionamento))
'    Call FileSetValue(nomeFile, SEZIONE, "FattoreDiCorrezioneKd", CStr(FattoreDiCorrezioneKd))
'    Call FileSetValue(nomeFile, SEZIONE, "FattoreDiCorrezioneKi", CStr(FattoreDiCorrezioneKi))
'
'End Sub
'
